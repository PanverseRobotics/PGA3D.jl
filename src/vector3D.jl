...
struct Vector3D{T<:Number} <: AbstractVector3D{T}
    vec::SVector{3,T}
    ...
    function Vector3D(vec::SVector{3,T}) where {T<:Number}
        new{T}(vec)
    end
end
...
get_e032(v::Vector3D) = v[1]
get_e013(v::Vector3D) = v[2]
get_e021(v::Vector3D) = v[3]
